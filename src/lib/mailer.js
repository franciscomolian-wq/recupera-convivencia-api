...
function resetHtml({ name, resetUrl }) {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:520px;margin:0 auto;color:#3C4043">
    <div style="background:#1A73E8;color:#fff;padding:20px 24px;border-radius:12px 12px 0 0">
      <div style="font-size:18px;font-weight:600">Recupera Convivencia</div>
      <div style="font-size:12px;opacity:.85;letter-spacing:1px;text-transform:uppercase">Recuperación de contraseña</div>
    </div>
    <div style="border:1px solid #DADCE0;border-top:none;border-radius:0 0 12px 12px;padding:24px">
      <p>Hola${name ? " " + name : ""},</p>
      <p>Recibimos una solicitud para restablecer tu contraseña en <b>Recupera Convivencia</b>. Haz clic en el botón para definir una nueva:</p>
      <p style="text-align:center;margin:28px 0">
        <a href="${resetUrl}" style="background:#1A73E8;color:#fff;text-decoration:none;padding:12px 24px;border-radius:999px;font-weight:600;display:inline-block">Restablecer mi contraseña</a>
      </p>
      <p style="font-size:12px;color:#5F6368">Si el botón no funciona, copia y pega este enlace:<br>
        <span style="word-break:break-all;color:#1A73E8">${resetUrl}</span></p>
      <p style="font-size:12px;color:#5F6368">El enlace vence en 1 hora. Si no solicitaste esto, ignora este correo; tu contraseña no cambiará.</p>
    </div>
  </div>`;
}

// Envía el correo de recuperación. Devuelve { sent, reason? }.
  ...
  try {
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        subject: "Recupera tu contraseña — Recupera Convivencia",
        htmlContent: resetHtml({ name, resetUrl }),
      }),
    });
    if (!res.ok) { console.error("Brevo reset", res.status, await res.text().catch(() => "")); return { sent: false, reason: "send-failed" }; }
    return { sent: true };
  } catch (e) { console.error("Brevo reset ex", e); return { sent: false, reason: "exception" }; }
}
